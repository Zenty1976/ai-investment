...
- targetAllocationPercent (integer 0–100): recommended portfolio weight after the trade, based on conviction and portfolio context. Consider current portfolio concentration and available cash.
- maximumAllocationPercent (integer 0–100, must be ≥ targetAllocationPercent): acceptable upper bound under favourable conditions (e.g. confirmed catalyst).
- sizingConfidence ("High" | "Medium" | "Low"): confidence in the sizing itself — not the investment case. Use Low when the position size depends on an unconfirmed catalyst or materially missing data.
- sizingReason (string, one sentence maximum): core rationale for this sizing only — not the investment case. Example: "Gradual entry limited to 4% until earnings guidance is confirmed."
